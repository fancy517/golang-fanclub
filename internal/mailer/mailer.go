...
type Mailer interface {
	Run()
	// GetQueue() chan any
	SendSettingCode(recipient, name, code string)
	SendActivationCode(recipient, name, code string)
	SendCongrats(recipient, name string)
	SendResetPasswordCode(recipient, name, code string)

	SendNewsletter(recipient, name, title, content string)

	SendTicket(name, email, subject, message string)

	SendReferral(recipient, name, refereeEmail, refereeName string, amount float64)
	SendRewardsAlertOnReferral(recipient, name, refereeEmail, refereeName string, amount float64)
}

type mailerImpl struct {
	dialer     *mail.Dialer
	sender     string
	mailQueue  chan MailPackage
	adminEmail string
}

var _ Mailer = (*mailerImpl)(nil)

func New(host string, port int, username, password, sender string, adminEmail string, queueCapacity int) Mailer {
	...
	return &mailerImpl{
		dialer:     dialer,
		sender:     sender,
		mailQueue:  make(chan MailPackage, queueCapacity),
		adminEmail: adminEmail,
	}
}

// func (m *mailerImpl) GetQueue() chan any {
// 	return m.mailQueue
// }
